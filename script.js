...
document.getElementById('saveProfileBtn').addEventListener('click', async () => {
    ...
    if (selectedAvatarFile) {
        ...
        const { data, error } = await client.storage.from('post-image').upload(fileName, selectedAvatarFile);
        if (error) {
            return alert('خطأ في رفع الصورة الشخصية: ' + error.message);
        }
        avatarUrl = client.storage.from('post-image').getPublicUrl(fileName).data.publicUrl;
    }
...
document.getElementById('submitPost').addEventListener('click', async () => {
    ...
    if (selectedPostFile) {
        ...
        const { data } = await client.storage.from('post-image').upload(fileName, selectedPostFile);
        if (data) {
            imageUrl = client.storage.from('post-image').getPublicUrl(fileName).data.publicUrl;
        }
    }
...
// جلب منشورات المستخدم فقط (للملف الشخصي)
...
// عرض المنشور بالصورة الشخصية لصاحبه
function renderPost(post, container) {
    ...
    let avatarSrc = post.avatar_url || DEFAULT_AVATAR;
...
async function fetchComments(postId) {
    ...
    list.innerHTML = '';
    comments?.forEach(c => {
        let avatarSrc = c.avatar_url || DEFAULT_AVATAR;
        list.innerHTML += `
            <div class="comment-item" style="display: flex; align-items: center; gap: 8px;">
                ...
                <div><span class="comment-user">${c.user_name}:</span> ${c.content}</div>
            </div>
        `;
    });
}
