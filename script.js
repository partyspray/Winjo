...
document.getElementById('saveProfileBtn').addEventListener('click', async () => {
    ...
    if (selectedAvatarFile) {
        ...
        // استخدام post-images
        const { data, error } = await client.storage.from('post-images').upload(fileName, selectedAvatarFile);
        if (error) {
            return alert('خطأ في رفع الصورة الشخصية: ' + error.message);
        }
        avatarUrl = client.storage.from('post-images').getPublicUrl(fileName).data.publicUrl;
    }
...
document.getElementById('submitPost').addEventListener('click', async () => {
    ...
    if (selectedPostFile) {
        ...
        // استخدام post-images
        const { data } = await client.storage.from('post-images').upload(fileName, selectedPostFile);
        if (data) {
